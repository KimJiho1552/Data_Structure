...
void deleteNode(linkedList_h* L, listNode* p) {
	...
	if (p == NULL) return;				//삭제할 노드가 없다면 삭제 연산 중단
	else if (L->head->link == NULL) {	//리스트에 노드가 한 개만 있는 경우
...
void insertMiddleNode(listNode** head, listNode* pre, char* x) {
	...
	if ((*head) == NULL) { //공백 리스트인 경우
		...
		newNode->link = (*head); //새 노드를 첫 번째 노드로 삽입
		(*head) = newNode;
	}
	else {
		newNode->link = pre->link; //포인터 pre의 노드 뒤에 새 노드 연결
...
void deleteNode(listNode** head, listNode* p) {
	...
	if (p == NULL) return;				//삭제할 노드가 없다면 삭제 연산 중단
	else if ((*head)->link == NULL) {	//리스트에 노드가 한 개만 있는 경우
		free((*head));					//첫 번째 노드를 메모리에서 해제하고
		(*head) = NULL;					//리스트 시작 포인터를 NULL로 설정
		return;
	}
	else {							//삭제할 노드 p의 선행자 노드를 포인터 pre를 이용해 찾음
		pre = (*head);
